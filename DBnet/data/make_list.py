...
def make_list(params):
    ...
    # img_path = os.path.abspath(os.path.join(datasets_root_path, "imgs/"))
    gt_path = os.path.join(datasets_root_path, "gt/")
    img_path = os.path.join(datasets_root_path, "imgs/")
    ...
    for img_name in img_list:
        ...
        gt_single_path = os.path.join(gt_path, str(img_name.split(".")[0]) + ".txt")
        ...
        if not params["use_curved_text"]:
            ...
            if re_result is not None:
                ...
                img_gt_list.append((os.path.join(img_path, img_name), gt_single_path))
    ...
    x_train, x_test = train_test_split(img_gt_list, test_size=0.1, random_state=0)     # 分割训练样本与测试样本
    ...
    with open(os.path.join(datasets_root_path, "train.txt"), mode="w", encoding="utf-8") as fd_train:
    ...
    with open(os.path.join(datasets_root_path, "test.txt"), mode="w", encoding="utf-8") as fd_test:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', help='experiment configuration filename', type=str, default='{}/utils/params.yaml'.format(base_dir))
    args = parser.parse_args()
    params = configs(args.cfg)
    print(params.DATASET.datasets_path)
    make_list(params.DATASET)
    # dataset_preprocess(params.DATASET)
